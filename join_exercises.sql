...
SELECT t.title, COUNT(t.title) as Total FROM employees e
    JOIN titles t ON e.emp_no = t.emp_no
    JOIN dept_emp de ON e.emp_no = de.emp_no
    JOIN departments d ON de.dept_no = d.dept_no
WHERE d.dept_name = 'Customer Service' AND t.to_date = '9999-01-01'
GROUP BY t.title ORDER BY Total DESC;

# TODO 5:
SELECT d.dept_name, CONCAT(e.first_name, ' ', e.last_name) AS dept_manager, s.salary FROM departments d
